...
class SymbolReferenceRegistry:
    ...
    def record_reference(self, context_symbol_id, referenced_symbol_id, reference_kind):
        # 生成一个关系键值，用于在字典中查找
        reference_key = (context_symbol_id, referenced_symbol_id, reference_kind)
...
class AstVisitorClient:
    ...
    def extract_code_between_lines(
        self, start_line, end_line, is_indent=True, is_code=True
    ):
        if is_code:
    ...
    def extract_code_from_file(
        self, file_path, start_line, end_line, is_indent=True, is_code=True
    ):
        if is_code:
            return '<CODE>{{"S":{0},"E":{1},"F":"{2}"}}</CODE>'.format(
    ...
    def recordSymbol(
        self, nameHierarchy, node_path="", tree_node=None, global_node=None
    ):
    ...
    def recordReferenceToUnsolvedSymhol(
        self, contextSymbolId, referenceKind, sourceRange
    ):
        pass

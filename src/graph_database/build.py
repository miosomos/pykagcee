...
def run_single(path, root, task_id, shallow, env_path_dict=None):
    ...
    script_path = os.path.join(
        env_path_dict["working_directory"], "run_index_single.py"
    )
...
def run_script_in_env(env_path, script_path, working_directory, script_args=None):
    ...
    if not os.path.exists(env_path):
        raise FileNotFoundError(
            "Python executable not found in the environment: {}".format(env_path)
        )
...
def build_graph_database(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(
                run_single, file_path, root_path, task_id, True, env_path_dict
            ): file_path
        ...
        for i, future in enumerate(as_completed(future_to_file)):
            ...
            try:
                ...
                msg = "`{}` generated an exception: `{}`".format(file_path, exc)
                print(msg)
